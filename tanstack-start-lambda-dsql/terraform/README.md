...
前提: mise、Bash、zip、AWS 認証情報。Node.js・pnpm・Terraform・AWS CLIは親ディレクトリの`mise.toml`で固定します。
...
DSQL の `deletion_protection_enabled = false` を apply した後、アセットバケットを空にし、`mise exec -- terraform -chdir=terraform destroy` で削除します。
